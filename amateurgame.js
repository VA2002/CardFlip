...
var clickCount9 = 0;
var clickCount10 = 0;
//a variable called gameScore equated with the amateur tier score in session storage (converted to number)
let gameScore = Number(sessionStorage.amateur);

//when game ends
function gameOver(){
  if(clickCount1 > 0 && clickCount2 > 0 && clickCount3 > 0 && clickCount4 > 0 && clickCount5 > 0 && clickCount6 > 0 && clickCount7 > 0 && clickCount8 > 0 && clickCount9 > 0 && clickCount10 > 0){
    //alert message displayed
    alert ("Game Over");
    //updating amateurscore in localstorage
    let finalScore = JSON.stringify(gameScore);
    sessionStorage.amateur = JSON.parse(finalScore);
    document.getElementById("score").innerHTML = "Final Score: " + sessionStorage.amateur;
    loggedinuser=sessionStorage.loggedInUsrName;
    user= JSON.parse(localStorage.getItem(loggedinuser));
    user.amateurscore=gameScore;
...
//Flip1 to Flip 10 describe the relation of each card with other cards on that page, along with scoring mechanism for each of these relations
